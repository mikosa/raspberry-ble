...
async def run(loop):
    ...
    my_service_name = "Hack me Service"
    server = BlessServer(name=my_service_name, loop=loop)
    server.read_request_func = read_request
    server.write_request_func = write_request

    await server.add_gatt(gatt)
    # Set manufacturer data for advertising
    battery_level=80
    server.manufacturer_data = {
        1: bytearray([battery_level])  # Manufacturer ID 1, data is battery level
    }
    await server.start()
